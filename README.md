...
5. Run the tests (The container must be running): `docker-compose exec -w /app/src app pytest -sv`
6. You can run `docker-compose exec app bash` to have a shell inside the container.
7. Example to run a single test:  `docker-compose exec  -w /app/src app pytest -sv tests/test_controller.py::test_player_in_turn`
